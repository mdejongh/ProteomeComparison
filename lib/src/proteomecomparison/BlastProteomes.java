...
public class BlastProteomes {
	...
	public static void run(ProteomeComparisonParams params, GenomeCmpConfig cfg) throws Exception {
		...
		ProteomeComparison res = new ProteomeComparison()
			...
			.withData1(data1new)
			.withData2(data2new);
		saveResult(params.getOutputWs(), params.getOutputId(), res, cfg.getObjectStorage());
	}
	...
	private static List<InnerFeature> extractProteome(String ws, String genomeId, 
			ObjectStorage objectStorage) throws Exception {
		UObject genomeObj = objectStorage.getObjects(
	...
	private static void saveResult(String ws, String id, ProteomeComparison res,
		...
		objectStorage.saveObjects(new SaveObjectsParams().withWorkspace(ws).withObjects(
				Arrays.asList(data)));
	}
